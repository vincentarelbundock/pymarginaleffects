...
def test_comparisons_01():
    unknown = comparisons(mod) \
        ...
        .select(["term", "contrast", "Species", "estimate_unknown"]) \
        .with_columns(pl.col("term").str.replace("_", "."))
    known = pl.read_csv("tests/r/test_statsmodels_quantreg_comparisons_01.csv")
    tmp = known.join(unknown, on = ["term", "contrast", "Species"], how = "left")
    assert known["estimate"].to_numpy() == approx(known["estimate_unknown"].to_numpy(), rel = 2e-1)
...
def test_comparisons_02():
    ...
    assert known["estimate"].to_numpy() == approx(unknown["estimate"].to_numpy(), rel = 2e-1)