...
def test_interaction_01_data():
    ...
    cmp_r = pl.read_csv(f"tests/r/test_comparisons_interaction_01.csv").sort(['term','contrast'])
...
#    dat=dat.with_columns(pl.col('am').cast(pl.String).cast(pl.Categorical)) 
    ...
    cmp = comparisons(
        mod_em,
        variables = {"cyl" : "all", "am" : "all"},
        newdata = datagrid(),
        cross = True
    )
    cmp_r_emmeans=pl.read_csv(f"tests/r/test_comparisons_interaction_emmeans.csv").sort(['estimate','contrast'])
    assert all( [ abs(round(em,5)) in cmp['estimate'] for em in cmp_r_emmeans['estimate'].round(5).abs() ] )
    assert all( [ abs(round(em,4)) in cmp['std_error'] for em in cmp_r_emmeans['SE'].round(4).abs() ] )
...
def test_variables_specified():
    ...
    with pytest.raises(Exception) as e_info:
        comparisons(mod, cross = True) # does not throw
