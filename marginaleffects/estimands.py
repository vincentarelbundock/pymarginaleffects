...
estimands = {
    ...
    "liftavg": lambda hi, lo, eps, x, y: prep([(np.mean(hi) - np.mean(lo)) / np.mean(lo)]),

    "expdydx": lambda hi, lo, eps, x, y: ((np.exp(hi) - np.exp(lo)) / np.exp(eps)) / eps,
    "expdydxavg": lambda hi, lo, eps, x, y: prep(np.mean(((np.exp(hi) - np.exp(lo)) / np.exp(eps)) / eps)),
