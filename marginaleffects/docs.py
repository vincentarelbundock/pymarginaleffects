...
* variables : (str, list, dictionary)
    Name of the variable whose {x} we want to plot on the y-axis.
...
"""
    ...
    def docstring_newdata_plot(x):
        return f"""
* newdata : (dataframe)
    When newdata is `None`, the grid is determined by the condition argument. When newdata is not `None`, the argument behaves in the same way as in the {x}() function.
"""
...
Max length of list is 4, with position meanings:
    1. x-axis.
    2. color.
